param(
    ...
    [string]$DbPath = ".semanticfs/bench/home_full.db",
    [string]$ConfigPath = "",
    [string]$FixturePath = "",
    [switch]$SkipBuild,
...
$resolvedConfigPath = if ([string]::IsNullOrWhiteSpace($ConfigPath)) {
    Join-Path $OutputDir "home_full.toml"
} else {
    $ConfigPath
}
$resolvedFixturePath = if ([string]::IsNullOrWhiteSpace($FixturePath)) {
    Join-Path $OutputDir "home_full_fixture.json"
} else {
    $FixturePath
}

if (-not $SkipBuild) {
    $buildScript = Join-Path $PSScriptRoot "build_home_full.ps1"
...
try {
    ...
    if (-not $SkipRelevance) {
        ...
        if (Test-Path ".semanticfs\bench\relevance_latest.json") {
            Copy-Item ".semanticfs\bench\relevance_latest.json" ".semanticfs\bench\home_full_relevance_latest.json" -Force
        }
    } elseif ($IncludeHeadToHead) {
    ...
    if ($IncludeHeadToHead) {
        ...
        if (Test-Path ".semanticfs\bench\head_to_head_latest.json") {
            Copy-Item ".semanticfs\bench\head_to_head_latest.json" ".semanticfs\bench\home_full_head_to_head_latest.json" -Force
        }
    }
}
finally {
